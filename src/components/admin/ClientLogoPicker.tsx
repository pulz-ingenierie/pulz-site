...
export default function ClientLogoPicker({ value, onChange }: { value?: string | null; onChange: (url: string | null) => void }) {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      await reload();
      onChange(pub.publicUrl.split('?')[0]); // sélectionne le nouveau logo
    } catch (e: any) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
        <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher un client…" style={{ flex: 1, minWidth: 200, padding: '9px 12px', border: '1px solid var(--line)', borderRadius: 8, fontFamily: 'inherit', fontSize: 14 }} />
        <label className="abtn" style={{ cursor: busy ? 'wait' : 'pointer' }}>
          {busy ? 'Ajout…' : '+ Ajouter un logo'}
          <input type="file" accept="image/*" style={{ display: 'none' }} disabled={busy} onChange={(e) => { const f = e.target.files?.[0]; if (f) upload(f); e.currentTarget.value = ''; }} />
        </label>
      </div>
